...
(
    ...
    [Parameter(HelpMessage = "Ubah versi Spotify yang direkomendasikan.")]
    [Alias("v")]
    [string]$version,

    [Parameter(HelpMessage = "Gunakan github.io mirror dan bukan raw.githubusercontent.")]
    [Alias("m")]
    [switch]$mirror,
    ...
    [Parameter(HelpMessage = 'Menyembunyikan podcast/episode/buku audio dari beranda.')]
    [switch]$podcasts_off,

    [Parameter(HelpMessage = 'Menyembunyikan bagian seperti Iklan dari beranda')]
    [switch]$adsections_off,
    
    [Parameter(HelpMessage = 'Jangan sembunyikan podcast/episode/buku audio dari beranda.')]
    [switch]$podcasts_on,
    
    [Parameter(HelpMessage = 'Block Spotify automatic updates.')]
    [switch]$block_update_on,
    
    [Parameter(HelpMessage = 'Jangan blokir Spotify automatic updates.')]
    [switch]$block_update_off,
    
    [Parameter(HelpMessage = 'Ubah batas untuk menghapus cache audio.')]
    [Alias('cl')]
    [int]$cache_limit,
    
    [Parameter(HelpMessage = 'Penghapusan instalasi Spotify MS secara otomatis jika ditemukan.')]
    [switch]$confirm_uninstall_ms_spoti,
    
    [Parameter(HelpMessage = 'Timpa versi Spotify yang sudah kedaluwarsa atau tidak didukung dengan versi yang direkomendasikan.')]
    [Alias('sp-over')]
    [switch]$confirm_spoti_recomended_over,
    
    [Parameter(HelpMessage = 'Copot pemasangan versi Spotify yang kedaluwarsa atau tidak didukung dan instal versi yang direkomendasikan.')]
    [Alias('sp-uninstall')]
    ...
    [Parameter(HelpMessage = 'Nonaktifkan autostart Spotify saat Windows boot.')]
    [switch]$DisableStartup,
    
    [Parameter(HelpMessage = 'Peluncuran Spotify otomatis setelah instalasi selesai.')]
    [switch]$start_spoti,
    
    [Parameter(HelpMessage = 'Fitur eksperimental yang dioperasikan oleh Spotify.')]
    [switch]$exp_spotify,

    [Parameter(HelpMessage = 'Enable top search bar.')]
    [switch]$topsearchbar,

    [Parameter(HelpMessage = 'nonaktifkan chip filter subfeed di beranda.')]
    [switch]$homesub_off,
...
function Get-Link {
    ...
    switch ($mirror) {
        $true { return "https://lioyuncho.github.io/spotify-kalong" + $endlink }
        default { return "https://raw.githubusercontent.com/kalong-spotify/main" + $endlink }
    }
}
...
If ($test_spa) {
    ...
    If ($patched_by_spotx -match 'patched by kalong community') {
        $zip.Dispose()    
